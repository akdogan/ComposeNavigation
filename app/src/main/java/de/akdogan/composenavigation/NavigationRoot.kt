...
data class ScreenConfiguration(
    ...
    companion object {
        ...
        val DETAIL_CONFIG = ScreenConfiguration(showUpArrow = true, "")
    }
}

interface ScreenConfigController {

    /**
     * !Should only be called from a [LaunchedEffect] or similar
     * @param update receives the current config and requires a new config
     */
    suspend fun updateConfiguration(
        update: ScreenConfiguration.() -> ScreenConfiguration
    )
}

val LocalScreenConfigController = staticCompositionLocalOf<ScreenConfigController> {
    error("ScreenConfigController was not configured yet!")
}
...
fun NavigationComponent(
    ...
    CompositionLocalProvider(value = LocalNavigator provides navigator) {
        CompositionLocalProvider(value = LocalScreenConfigController provides screenConfigController) {

            NavHost(navController = navHostController, startDestination = NavigationRoutes.Welcome) {
                composable<NavigationRoutes.Welcome> {
                    WelcomeScreen()
                    LaunchedEffect(key1 = Unit) {
                        screenConfigController.updateConfiguration { ScreenConfiguration.WELCOME_CONFIG }
                    }
                }
                composable<NavigationRoutes.List> {
                    ...
                    LaunchedEffect(key1 = Unit) {
                        screenConfigController.updateConfiguration { ScreenConfiguration.LIST_CONFIG }
                    }
                }
                composable<NavigationRoutes.Detail> {
                    ...
                    LaunchedEffect(key1 = Unit) {
                        screenConfigController.updateConfiguration { ScreenConfiguration.DETAIL_CONFIG }
                    }
                }
            }
        }
